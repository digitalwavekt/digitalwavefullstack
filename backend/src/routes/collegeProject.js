...
export default function CollegeProjects() {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/college-project/${id}/status`, {
        method: 'PUT',
        headers: authHeaders,
              ...
              {filteredProjects.length === 0 ? (
                ...
                filteredProjects.map((project) => (
                  <tr key={project.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium border ${statusColors[project.status] || 'bg-gray-500/10 text-gray-400 border-gray-500/20'
                          }`}
                      >
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              {[
                ...
                ['Branch & Year', `${selectedProject.branch || '-'} - ${selectedProject.year || '-'} Year`],
                ['Project Title', selectedProject.project],
                ['Tech Stack', selectedProject.stack],
                ['Amount', `₹${Number(selectedProject.amount || 0).toLocaleString()}`],
              ].map(([label, value]) => (
